...
#define ARC_SWEEPNET1DDET "FASTER-NN"
#define CLA_SWEEPNET1DDET 2
#define PCLA_SWEEPNET1DDET 1
...
{	// -a is also reserved for the rand gen's seed
	...
	int		mode; // Flag: -md=training -md=inference THINK ABOUT THIS
	uint64_t	imgLeBor; // Flag: imgBor left border
	uint64_t	imgRiBor; // Flag: imgBor right border
...
#ifdef _RSDAI
void		RSDMuStat_output2BufferFullExtended (RSDMuStat_t * RSDMuStat, double windowCenter, double windowStart, double windowEnd, double muVar, double muSfs, double muLd, double mu, double label1, double label2, double label3, double label4, int classes);
...
typedef struct
{
	int64_t 	size; // this is gridPointSize
	double *	positions;
	
	
	float *		nnScores0; // CNN scores	
	float * 	nnScores1; // used for recomb. signal only
	...
	double 		nnScore1Final;
	double 		nnScore2Final;
	double 		nnScore3Final;	
	double  	muVarScoreFinal;
	double 		muSfsScoreFinal;
	double 		muLdScoreFinal;
	double 		muScoreFinal;
...
void 			RSDGridPoint_reduce 		(RSDGridPoint_t * RSDGridPoint, int op, char * networkArchitecture);
...
typedef struct 
{
	...
	int8_t * 	data;
	int8_t *	dataT; // tmp for pixel reordering TODO-AI-x is this used? check-cleanup
	int8_t * 	incomingSNP;
	...
	double		avgImageWidthBP;
	uint8_t	*	byteBuffer; // this is used in BIN conversion
	uint64_t *	longIntBuffer; // this is used in BIN conversion
