...
RSD-DEF: Standard RAiSD execution using only the mu-statistic (no CNN)\n");
	...
	fprintf(fp, "\t-cl4\tProvides four pairings (using '=') between class labels and corresponding folder names in the RAiSD_Images.some-runID\n\t\tdirectory (provided through -I). They are organized into two groups, 1 and 2, to carry out two simultaneous binary\n\t\tclassifications. This is only supported by SweepNetRecombination.\n\t\tSyntax: \"-cl4 label1=folderA label2=folderB label3=folderC label4=folderD\", where label1 through 4 are the label names,\n\t\tand folderA through D are in the directory specified via -I. Label1 and label2 are Group 1 while label3 and label4 are Group 2. \n");  //tests followed by an equal number of pairings (using '=') between the class labels and\n\t\tthe 
	...
	//fprintf(fp, "\t-mdl\tProvides a path to the directory where the trained CNN model is stored. This should be the RAiSD_Model.some-runID, where\n\t\tsome-runID is the run ID of some previous run in model generation (CNN training) mode.\n");
	//fprintf(fp, "\t-I\tIn this mode (model test), -I provides the path to the directory where the test data is stored. This can be\n\t\tdirectory RAiSD_Images.some-runID, where some-runID is the run ID of some previous run in data generation mode.\n\t\tThis directory should contain one folder with test data for each class.\n");	
	//fprintf(fp, "\t-clp\tProvides the number of class tests followed by an equal number of pairings (using '=') between the class labels and the\n\t\tcorresponding folder names in the RAiSD_Images.some-runID directory (provided through -I).\n\t\tExample: \"-clp 2 label1=folderA label2=folderB\", where label1 and label2 are the label names, and folderA and folderB\n\t\tare in the directory specified via -I.\n");
...
void RSDVersions(FILE * fp)
{
	...
	fprintf(fp, " %*d. RAiSD v%d.%d (Mar  4, 2024): RAiSD-AI release (data generation, CNN training and testing, and CNN-based scans)\n", strlen, releaseIndex++, majorIndex, minorIndex++);
...
void RSDCommandLine_init(RSDCommandLine_t * RSDCommandLine)
{
	...
	//RSDCommandLine->imgGen = 0; 
	RSDCommandLine->imgLeBor = 0ull; 
	RSDCommandLine->imgRiBor = 0ull; 
#endif
}
	...
	for(i=1; i<argc; ++i)
	{
		...
		{ 
			...
			if (i!=argc-1 && argv[i+1][0]!='-')
			{
				...
				if(!strcmp(tstring2, "RSD-DEF")) // default, no CNN use
				{
					RSDCommandLine->opCode = OP_DEF;
				}
				else
				{
					if(!strcmp(tstring2, "IMG-GEN")) // create images from a single target from each simulation, no CNN used here
					{
		...
		{ 
			...
			if (i!=argc-1 && argv[i+1][0]!='-')
			{
				...
				// TODO: add check that provided folder is valid trained folder or work only with files.. ask Hanqing to fix that.
		...
		{ 
			...
			if (i!=argc-1 && argv[i+1][0]!='-')
			{
				...
				RSDCommandLine->classLabelList = (char**)malloc(sizeof(char*)*RSDCommandLine->numberOfClasses); // this must be the true class
				assert(RSDCommandLine->classLabelList!=NULL);
				
				RSDCommandLine->classPathList = (char**)malloc(sizeof(char*)*RSDCommandLine->numberOfClasses);
				...
				for(j=0;j<RSDCommandLine->numberOfClasses;j++)
				{
					if (i!=argc-1 && argv[i+1][0]!='-')
					{
						RSDCommandLine->classLabelList[j] = (char*)malloc(sizeof(char)*STRING_SIZE);
		...
		// TODO: add check about ics be in range
		// think about default number of images 0, or 1?
				
				
		if(!strcmp(argv[i], "-imgBor")) 
		{ 
			//flagCheck (argv, i, flagVector, 2);
			...
			if (i<argc-2 && i!=argc-1 && argv[i+1][0]!='-' && argv[i+2][0]!='-')
			{
				...
				RSDCommandLine->imgLeBor = (uint64_t) bor;
				
				bor = atof(argv[++i]);
				RSDCommandLine->imgRiBor = (uint64_t) bor;
			}
			else
			{
		...
		{ 
			...
			if (i!=argc-1 && argv[i+1][0]!='-')
			{	
				//RSDCommandLine->numOfPositiveClasses = 1;
				//RSDCommandLine->positiveClassIndex = (int *)rsd_realloc(RSDCommandLine->positiveClassIndex, sizeof(int)*);
				//RSDCommandLine->positiveClassIndex = (int)atoi(argv[++i]);
		...
		{ 
			...
			if (i!=argc-1 && argv[i+1][0]!='-')
			{
				...
				RSDCommandLine->numberOfClasses = 4; //numberOfClasses;
...
//	if(RSDCommandLine->opCode != OP_TRAIN_CNN && RSDCommandLine->opCode != OP_TEST_CNN)
	...
	*/
	...
	{
		fprintf(stderr, "\nERROR: Remove -type. It is only used in IMG-GEN mode for data generation!\n\n");
		exit(0);	
	}	
	...
	/*if ((!(flagVector[POSITIVE_CLASS_FLAG_INDEX])) && (RSDCommandLine->opCode==OP_USE_CNN))
	{
		fprintf(stderr, "\nERROR: Missing input parameter -pci that is required when \"-op SWP-SCN\" is used!\n\n");
		exit(0);	
	}*/
	...
	/*if(info_exists==1 || info_exists==-1) 
	{
		...
		if(info_exists==1 && RSDCommandLine->overwriteOutput==0)
		{
			fprintf(stderr, "\nERROR: Output info file %s exists. Use -f to overwrite it.\n\n", tstring); 
			exit(0);
		}
