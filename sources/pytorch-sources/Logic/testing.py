...
def test_model(model, test_loader, platform, dense=False):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(test_loader):
            ...
            if dense:
                input_distances = inputs[0, 0, 0, :]
                distances = input_distances * 200.0     
            outputs = model(inputs.float()) # input needs float, but tensor is double for precision of distances
    ...
    if dense:
        return path_list, output_list, label_list, inference_time, distances
    else:
