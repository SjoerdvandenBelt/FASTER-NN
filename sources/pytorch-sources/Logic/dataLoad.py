...
def snp_file_loader(path):
       ...
       # print(num_cols)
        ...
        ## reorder based on hamming distance per row!#
        # hamming_distances = np.count_nonzero(np.not_equal(np.expand_dims(snp_matrix[0,:],0), snp_matrix[:,:]), axis=1)
        # hamming_order = np.argsort(hamming_distances)
        # snp_matrix = snp_matrix[hamming_order,:]
        ###
        
        snp_tensor = torch.from_numpy(snp_matrix).float()

        bp_array = np.frombuffer(data, dtype=np.float64, offset=16+ints_per_col*num_cols, count=num_cols)
        # print("Chrom len: ", np.sum(bp_array))
        bp_tensor = torch.from_numpy(bp_array.copy())
        bp_tensor = torch.broadcast_to(bp_tensor, snp_tensor.shape)

        sample = torch.stack([bp_tensor, bp_tensor, snp_tensor], dim=0).double()
       # print(sample.shape)
        return sample, target_pos
