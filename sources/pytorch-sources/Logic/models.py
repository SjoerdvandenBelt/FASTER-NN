...
class FAST_NN(torch.nn.Module):
    ...
    def __init__(self, H, W, outputs, channels):
        ...
        self.fc1 = torch.nn.Linear(conv3_channels, 32)
        self.out_relu = torch.nn.ReLU()
        self.out_conv = torch.nn.Conv1d(in_channels=32, out_channels=32, groups=32, kernel_size=out_pool3[1], bias=False) # replace out avg pool with out conv
        self.out_conv.weight = torch.nn.Parameter(torch.ones_like(self.out_conv.weight)/self.out_conv.weight.shape[2], requires_grad=False) # set out_conv params to make it averaging
        self.fc2 = torch.nn.Linear(32, 2*self.outputs)
        self.softmax = torch.nn.Softmax(dim=1) 
...
class FAST_NN_quantized(torch.nn.Module):
    
    def __init__(self, H, W, outputs, channels):
        super(FAST_NN_quantized, self).__init__()
        ...
        # set out_conv params to make it averaging
        self.out_conv.weight = torch.nn.Parameter(torch.ones_like(self.out_conv.weight)/self.out_conv.weight.shape[2], requires_grad=False) 
...
class FAST_NN_dense(torch.nn.Module):    
    def __init__(self, H, W, outputs, channels):
        super(FAST_NN_dense, self).__init__()
        ...
        pool1_stride = 1 # down from 2
        pool2_stride = 1 # down from 2
        pool3_stride = 1 # down from 2
        ...
        self.conv2 = torch.nn.Conv1d(conv1_channels, conv2_channels, kernel_size=conv2_kernel, stride=conv2_stride, padding=0, dilation=2)
        self.pool2 = torch.nn.MaxPool1d(pool2_kernel, pool2_stride, dilation=2)
        self.relu2 = torch.nn.ReLU()
        
        self.conv3 = torch.nn.Conv1d(conv2_channels, conv3_channels, kernel_size=conv3_kernel, stride=conv3_stride, padding=0, dilation=4)
        self.pool3 = torch.nn.MaxPool1d(pool3_kernel, pool3_stride, dilation=4)
        self.relu3 = torch.nn.ReLU()
        
        
        # compute output size for strided model
        W_window = 512
        out_conv1 = self.compute_out_shape((H, W_window), (conv1_kernel, conv1_kernel), (conv1_stride, conv1_stride))
        out_pool1 = self.compute_out_shape(out_conv1, (pool1_kernel, pool1_kernel), (2, 2))
        out_conv2 = self.compute_out_shape(out_pool1, (conv2_kernel, conv2_kernel), (conv2_stride, conv2_stride))
        out_pool2 = self.compute_out_shape(out_conv2, (pool2_kernel, pool2_kernel), (2, 2))
        out_conv3 = self.compute_out_shape(out_pool2, (conv3_kernel, conv3_kernel), (conv3_stride, conv3_stride))
        out_pool3 = self.compute_out_shape(out_conv3, (pool3_kernel, pool3_kernel), (2, 2))
        
        # compute output size for dilated model
        out_conv1_d = self.compute_out_shape((H, W), (conv1_kernel, conv1_kernel), (1, 1), (1, 1))
        out_pool1_d = self.compute_out_shape(out_conv1_d, (pool1_kernel, pool1_kernel), (1, 1), (1, 1))
        out_conv2_d = self.compute_out_shape(out_pool1_d, (conv2_kernel, conv2_kernel), (1, 1), (2, 2))
        out_pool2_d = self.compute_out_shape(out_conv2_d, (pool2_kernel, pool2_kernel), (1, 1), (2, 2))
        out_conv3_d = self.compute_out_shape(out_pool2_d, (conv3_kernel, conv3_kernel), (1, 1), (4, 4))
        out_pool3_d = self.compute_out_shape(out_conv3_d, (pool3_kernel, pool3_kernel), (1, 1), (4, 4))
        out_conv_out_d = self.compute_out_shape(out_pool3_d, (out_pool3[1], out_pool3[1]), (1, 1), (8, 8))
        self.W_out = out_conv_out_d[1]
        
        # init output layers
        self.fc1 = torch.nn.Linear(conv3_channels, 32)
        self.out_relu = torch.nn.ReLU()
        self.out_conv = torch.nn.Conv1d(in_channels=32, out_channels=32, groups=32, dilation=8, kernel_size=out_pool3[1], bias=False) # replace out avg pool with out conv
        self.out_conv.weight = torch.nn.Parameter(torch.ones_like(self.out_conv.weight)/self.out_conv.weight.shape[2], requires_grad=False) # set out_conv params to make it averaging
        self.fc2 = torch.nn.Linear(32, 2*self.outputs)
        self.softmax = torch.nn.Softmax(dim=1) 
        
    def compute_out_shape(self, I, k, s, d=(1, 1)):
        out_H = floor(((I[0] - d[0]*(k[0] - 1) - 1) / s[0]) + 1)
        out_W = floor(((I[1] - d[1]*(k[1] - 1) - 1) / s[1]) + 1)
        return (out_H, out_W)
    ...
    def forward(self, x):
        ...
        x = torch.movedim(x, 1, -1) # channels to last dim
        x = self.fc1(x)
        x = self.out_relu(x)
        x = torch.movedim(x, -1, 1) # channels to second dim
        x = self.out_conv(x)
        x = torch.movedim(x, -1, 1) # width to second dim (B, W, C, H)
        x = x.flatten(2) # (B, W, ..)
        x = self.fc2(x) # (B, W, [2*out])
        
        # x = x.reshape(-1, self.W_out, self.outputs, 2)
        x = x.permute(0, 2, 1)
        x = x.reshape(x.shape[0], -1, self.outputs, 2)
        # x = x[:, 1, :, :] # select first one only to compare with non dense
        x = x.squeeze(2)
        return x
    
    
class FAST_NN_dense_buffer(torch.nn.Module):    
    def __init__(self, H, W, outputs, channels):
        super(FAST_NN_dense_buffer, self).__init__()
        ...
        # init buffers
        # buffer input is dilation*(kernel-1)
        self.conv1_buffer = torch.zeros((1, 2, 1))
        self.pool1_buffer = torch.zeros((1, 80, 1))
        self.conv2_buffer = torch.zeros((1, 80, 2))
        self.pool2_buffer = torch.zeros((1, 80, 2))
        self.conv3_buffer = torch.zeros((1, 80, 4))
        self.pool3_buffer = torch.zeros((1, 80, 4))
        self.conv_out_buffer_size = 8*(out_pool3[1]-1)
        self.conv_out_buffer = torch.zeros((1, 32, self.conv_out_buffer_size))
        
        # compute output size for dilated model
        out_conv1_d = self.compute_out_shape((H, W+1), (conv1_kernel, conv1_kernel), (1, 1), (1, 1))
        out_pool1_d = self.compute_out_shape((out_conv1_d[0], out_conv1_d[1]+1), (pool1_kernel, pool1_kernel), (1, 1), (1, 1))
        out_conv2_d = self.compute_out_shape((out_pool1_d[0], out_pool1_d[1]+2), (conv2_kernel, conv2_kernel), (1, 1), (2, 2))
        out_pool2_d = self.compute_out_shape((out_conv2_d[0], out_conv2_d[1]+2), (pool2_kernel, pool2_kernel), (1, 1), (2, 2))
        out_conv3_d = self.compute_out_shape((out_pool2_d[0], out_pool2_d[1]+4), (conv3_kernel, conv3_kernel), (1, 1), (4, 4))
        out_pool3_d = self.compute_out_shape((out_conv3_d[0], out_conv3_d[1]+4), (pool3_kernel, pool3_kernel), (1, 1), (4, 4))
        out_conv_out_d = self.compute_out_shape((out_pool3_d[0], out_pool3_d[1]+self.conv_out_buffer_size), (out_pool3[1], out_pool3[1]), (1, 1), (8, 8))
        self.W_out = out_conv_out_d[1]
    ...
    def forward(self, x):    
        ...
        x = torch.cat([self.conv1_buffer, x], dim=2)
        self.conv1_buffer = x[:,:,-1:]
        x = self.conv1(x)
        
        x = torch.cat([self.pool1_buffer, x], dim=2)
        self.pool1_buffer = x[:,:,-1:]
        x = self.pool1(x)
        x = self.relu1(x)
        
        x = torch.cat([self.conv2_buffer, x], dim=2)
        self.conv2_buffer = x[:,:,-2:]
        x = self.conv2(x)

        x = torch.cat([self.pool2_buffer, x], dim=2)       
        self.pool2_buffer = x[:,:,-2:]
        x = self.pool2(x)
        x = self.relu2(x)
        
        x = torch.cat([self.conv3_buffer, x], dim=2)
        self.conv3_buffer = x[:,:,-4:]
        x = self.conv3(x)
        
        x = torch.cat([self.pool3_buffer, x], dim=2)
        self.pool3_buffer = x[:,:,-4:]
        x = self.pool3(x)
        x = self.relu3(x)
        ...
        x = torch.cat([self.conv_out_buffer, x], dim=2)
        self.conv_out_buffer = x[:,:,-self.conv_out_buffer_size:]
        x = self.out_conv(x)
...
class SweepNet1DDetGrouped(torch.nn.Module):
    
    def __init__(self, H, W, outputs, channels):
        super(SweepNet1DDetGrouped, self).__init__()
    ...
    def forward(self, x):
        ...
        if self.channels == 1:
            ...
            x = x[:,0:3,:,:] 
        x_ungrouped = torch.mean(x, dim=2)     
        x = self.avg_group_pool(x)
        x = torch.permute(x, (0, 2, 1, 3))
        x = torch.reshape(x, (x.shape[0]*self.groups, x.shape[2], x.shape[3]))       

        # subtract overall DAF
        # x[:, -1, :] = (x[:, -1, :] - x_ungrouped[:, -1, :].repeat_interleave(self.groups, dim=0))# *(self.height//self.groups)
        # add total daf as channel
        # x = torch.cat((x, x_ungrouped[:,-1,:].unsqueeze(1).repeat_interleave(self.groups, dim=0)), dim=1)
        ...
        # x_a = self.group_apool(x_p).repeat_interleave(self.groups, dim=2).permute(0,2,1,3).reshape((x.shape))
        # x_p = torch.cat((x, x_m, x_a), dim=1)
...
class FAST_NN_det_dense(torch.nn.Module):
        
    def __init__(self, H, W, outputs, channels):
        super(FAST_NN_det_dense, self).__init__()
        ...
        conv2_stride = 1
        conv3_stride = 1
        conv4_stride = 1 # was 2
        conv5_stride = 1 # was 2
        conv6_stride = 1 # was 2
        ...
        pool1_stride = 1 # was 2
        pool2_stride = 1 # was 2
        pool3_stride = 1 # was 2
                
        self.channels = channels
        self.outputs = outputs
        self.conv1 = torch.nn.Conv1d(channels, conv1_channels, kernel_size=conv1_kernel, stride=conv1_stride, padding=0) # (kw-1)/2 = 2/2=1
        self.pool1 = torch.nn.MaxPool1d(pool1_kernel, pool1_stride) # 1+(1/2)=1.5
        self.relu1 = torch.nn.ReLU()
        
        self.conv2 = torch.nn.Conv1d(conv1_channels, conv2_channels, kernel_size=conv2_kernel, stride=conv2_stride, dilation=2, padding=0) # takes 4+(2*2)=8 to get first good output
        self.pool2 = torch.nn.MaxPool1d(pool2_kernel, pool2_stride, dilation=2) # takes 8+(1*2)=10 to get first good output
        self.relu2 = torch.nn.ReLU()
        
        self.conv3 = torch.nn.Conv1d(conv2_channels, conv3_channels, kernel_size=conv3_kernel, stride=conv3_stride, dilation=4, padding=0) # takes 10+(3*4)
        self.pool3 = torch.nn.MaxPool1d(pool3_kernel, pool3_stride, dilation=4)
        self.relu3 = torch.nn.ReLU()
        
        self.conv4 = torch.nn.Conv1d(conv3_channels, conv4_channels, kernel_size=conv4_kernel, stride=conv4_stride, dilation=8, padding=0) # padding was 3
        self.relu4 = torch.nn.ReLU()
        
        self.conv5 = torch.nn.Conv1d(conv4_channels, conv5_channels, kernel_size=conv5_kernel, stride=conv5_stride, dilation=16, padding=0) # padding was 3
        self.relu5 = torch.nn.ReLU()
        
        self.conv6 = torch.nn.Conv1d(conv5_channels, conv6_channels, kernel_size=conv6_kernel, stride=conv6_stride, dilation=32, padding=0) # padding was 3
        self.relu6 = torch.nn.ReLU()
        ...
        out_conv3 = self.compute_out_shape(out_pool2, (conv3_kernel, conv3_kernel), (conv3_stride, conv3_stride))
        out_pool3 = self.compute_out_shape(out_conv3, (pool3_kernel, pool3_kernel), (2, 2))
        out_conv4 = self.compute_out_shape((out_pool3[0], out_pool3[1]), (conv4_kernel, conv4_kernel), (2, 2))
        out_conv5 = self.compute_out_shape((out_conv4[0], out_conv4[1]), (conv5_kernel, conv5_kernel), (2, 2))
        out_conv6 = self.compute_out_shape((out_conv5[0], out_conv5[1]), (conv5_kernel, conv5_kernel), (2, 2))
        
        # init output layers
        self.fc = torch.nn.Conv1d(32, 2*self.outputs, kernel_size=out_conv6[1], stride=1, dilation=64, padding=0)
